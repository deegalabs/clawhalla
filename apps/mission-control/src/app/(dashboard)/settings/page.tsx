...
type SettingsTab = 'profile' | 'general' | 'connection' | 'database' | 'vault' | 'gateway' | 'memory' | 'about';
...
function SettingsPageInner() {
  ...
  // Content / auto-publish state
  const [autoPublishOnApprove, setAutoPublishOnApprove] = useState(true);

  // Profile state
  const [profile, setProfile] = useState({ name: '', callName: '', pronouns: '', timezone: '', notes: '', context: '' });
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      if (data.ok && data.profile) {
        setProfile({
          ...
          callName: data.profile.callName || '',
    ...
    } catch { /* profile not configured yet */ }
  }, []);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        setProfileMsg(`Saved! Synced to ${data.copied} agent(s).`);
      } else {
  ...
  useEffect(() => {
    fetchSecrets(); fetchGateway(); fetchMemoryConfig(); fetchProfile();
    // Load auto-publish setting
    fetch('/api/settings?key=auto_publish_on_approve').then(r => r.json()).then(d => {
      if (d.value !== undefined) setAutoPublishOnApprove(d.value !== 'false');
    }).catch(() => {});
  }, [fetchSecrets, fetchGateway, fetchMemoryConfig, fetchProfile]);
    ...
    <div className="flex flex-col h-[calc(100vh-7rem)] gap-3">
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <div className="flex gap-0.5 bg-[#111113] rounded-lg p-0.5 border border-[#1e1e21]">
          {(['profile', 'general', 'connection', 'database', 'vault', 'gateway', 'memory', 'about'] as SettingsTab[]).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-2.5 py-1 text-[11px] rounded capitalize ${tab === t ? 'bg-[#1e1e21] text-gray-100' : 'text-gray-500 hover:text-gray-300'}`}>
              {t === 'profile' ? '👤 Profile' : t === 'general' ? '⚙️ General' : t === 'connection' ? '🔌 Connection' : t === 'database' ? '💾 Database' : t === 'vault' ? '🔒 Vault' : t === 'gateway' ? '🌐 Gateway' : t === 'memory' ? '🧠 Memory' : 'ℹ️ About'}
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        {tab === 'profile' && (
          <div className="max-w-2xl space-y-4">
            <div className="bg-[#111113] rounded-lg border border-[#1e1e21] p-4">
              <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-3 font-medium">Your Profile</div>
              <div className="text-[9px] text-gray-600 mb-4">Tell your agents about yourself. This information is saved to USER.md and shared with all agents in your workspace.</div>
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[9px] text-gray-600 mb-0.5">Name</label>
                    <input type="text" value={profile.name} onChange={e => setProfile({ ...profile, name: e.target.value })}
                      placeholder="Your full name"
                      className="w-full px-3 py-1.5 bg-[#0a0a0b] border border-[#1e1e21] rounded text-[11px] text-gray-200 focus:outline-none focus:border-amber-500" />
                  </div>
                  <div>
                    <label className="block text-[9px] text-gray-600 mb-0.5">What to call me</label>
                    <input type="text" value={profile.callName} onChange={e => setProfile({ ...profile, callName: e.target.value })}
                      placeholder="e.g. Dan, Boss, Captain"
                      className="w-full px-3 py-1.5 bg-[#0a0a0b] border border-[#1e1e21] rounded text-[11px] text-gray-200 focus:outline-none focus:border-amber-500" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[9px] text-gray-600 mb-0.5">Pronouns <span className="text-gray-700">(optional)</span></label>
                    <input type="text" value={profile.pronouns} onChange={e => setProfile({ ...profile, pronouns: e.target.value })}
                      placeholder="e.g. he/him, she/her, they/them"
                      className="w-full px-3 py-1.5 bg-[#0a0a0b] border border-[#1e1e21] rounded text-[11px] text-gray-200 focus:outline-none focus:border-amber-500" />
                  </div>
                  <div>
                    <label className="block text-[9px] text-gray-600 mb-0.5">Timezone</label>
                    <input type="text" value={profile.timezone} onChange={e => setProfile({ ...profile, timezone: e.target.value })}
                      placeholder="e.g. UTC-3, America/Sao_Paulo"
                      className="w-full px-3 py-1.5 bg-[#0a0a0b] border border-[#1e1e21] rounded text-[11px] text-gray-200 focus:outline-none focus:border-amber-500" />
                  </div>
                </div>
                <div>
                  <label className="block text-[9px] text-gray-600 mb-0.5">Notes</label>
                  <input type="text" value={profile.notes} onChange={e => setProfile({ ...profile, notes: e.target.value })}
                    placeholder="Quick notes about yourself for agents"
                    className="w-full px-3 py-1.5 bg-[#0a0a0b] border border-[#1e1e21] rounded text-[11px] text-gray-200 focus:outline-none focus:border-amber-500" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#111113] rounded-lg border border-[#1e1e21] p-4">
              <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-3 font-medium">Context</div>
              <div className="text-[9px] text-gray-600 mb-2">What do you care about? What projects are you working on? What are your preferences? The more your agents know, the better they can help.</div>
              <textarea value={profile.context} onChange={e => setProfile({ ...profile, context: e.target.value })}
                rows={6}
                placeholder="I'm a developer working on... I prefer... My current focus is..."
                className="w-full px-3 py-2 bg-[#0a0a0b] border border-[#1e1e21] rounded text-[11px] text-gray-200 focus:outline-none focus:border-amber-500 resize-y font-mono leading-relaxed" />
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {profileMsg && <span className={`text-[10px] ${profileMsg.includes('Saved') ? 'text-green-400' : 'text-red-400'}`}>{profileMsg}</span>}
            </div>
          </div>
        )}
        ...
        {tab === 'general' && (
          <div className="max-w-2xl space-y-4">
            ...
            {/* Content Publishing */}
            <div className="bg-[#111113] rounded-lg border border-[#1e1e21] p-4">
              <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-3 font-medium">Content Publishing</div>
              <div className="space-y-2">
                <div className="flex items-center justify-between py-1.5">
                  <div>
                    <div className="text-[11px] text-gray-300">Auto-publish on approve</div>
                    <div className="text-[9px] text-gray-600">Automatically publish to the platform when a draft is approved (via Telegram or UI)</div>
                  </div>
                  <button
                    onClick={async () => {
                      const next = !autoPublishOnApprove;
                      setAutoPublishOnApprove(next);
                      await fetch('/api/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: 'auto_publish_on_approve', value: next ? 'true' : 'false' }) });
                    }}
                    className={`w-8 h-4 rounded-full relative cursor-pointer ${autoPublishOnApprove ? 'bg-amber-500' : 'bg-[#2a2a2d]'}`}>
                    <div className="absolute w-3 h-3 rounded-full bg-white top-0.5 transition-all"
                      style={{ left: autoPublishOnApprove ? '17px' : '2px' }} />
                  </button>
                </div>
              </div>
            </div>
