...
import { campaigns, campaignContacts, approvals } from '@/lib/schema';
...
import { broadcastNotification } from '@/lib/events';
...
export async function DELETE(req: NextRequest, ctx: RouteContext) {
  ...
  // Only users can delete campaigns
  if (auth.type === 'agent') {
    return NextResponse.json({ ok: false, error: 'Agents cannot delete campaigns. Request approval.' }, { status: 403 });
  }
  ...
  if (isRunning(id)) abortCampaign(id);
...
export async function POST(req: NextRequest, ctx: RouteContext) {
  ...
  if (action === 'send') return handleSend(id, auth);
  if (action === 'pause') return handlePause(id, auth);
...
async function handleSend(id: string, auth: { type: string; agentId?: string }) {
  const [campaign] = await db.select().from(campaigns).where(eq(campaigns.id, id));
  if (!campaign) return NextResponse.json({ ok: false, error: 'Campaign not found' }, { status: 404 });

  // AGENTS: cannot send directly — must go through approval gate
  if (auth.type === 'agent') {
    const agentId = auth.agentId || 'unknown-agent';

    // Check if there's already a pending approval for this campaign
    const existing = await db.select({ id: approvals.id })
      .from(approvals)
      .where(and(
        eq(approvals.taskId, `campaign-send:${id}`),
        eq(approvals.status, 'pending')
      ));

    if (existing.length > 0) {
      return NextResponse.json({ ok: false, error: 'Approval already pending for this campaign' }, { status: 409 });
    }

    // Create approval request
    const approvalId = `apr_${crypto.randomUUID()}`;
    const contactCount = campaign.totalContacts;

    await db.insert(approvals).values({
      id: approvalId,
      taskId: `campaign-send:${id}`,
      requestedBy: agentId,
      approver: 'daniel',
      status: 'pending',
      command: `campaign:send:${id}`,
      reason: `Send campaign "${campaign.name}" to ${contactCount} contacts (${campaign.subject})`,
      createdAt: new Date(),
    });
    ...
    broadcastNotification({
      type: 'approval',
      title: 'Campaign Send Request',
      body: `${agentId} wants to send "${campaign.name}" to ${contactCount} contacts`,
      icon: '📧',
      href: `/campaigns`,
      agentId,
      priority: 'high',
    ...
    return NextResponse.json({
      ok: true,
      message: `Approval requested. Waiting for human approval to send to ${contactCount} contacts.`,
      approvalId,
    });
  }

  // USERS: send directly (with rate limiting)
...
async function handlePause(id: string, auth: { type: string }) {
  // Only users can pause
  if (auth.type === 'agent') {
    return NextResponse.json({ ok: false, error: 'Agents cannot pause campaigns' }, { status: 403 });
  }
